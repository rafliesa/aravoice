...
export default function NewsCard({ news }: NewsCardProps) {
  ...
  return (
    ...
    >
      <article
        id={`berita-${news.id}`}
        className="h-full overflow-hidden rounded-lg border border-zinc-200 bg-white transition-shadow hover:border-zinc-300 hover:shadow-lg"
      >
        {news.cover_image ? (
        ...
        <div className="p-6">
          ...
          <h3 className="mt-3 text-xl font-bold leading-snug group-hover:text-[#8A5100]">
            {news.title}
          </h3>
          ...
          <div className="mt-6 flex items-center justify-between border-t border-zinc-100 pt-4">
            ...
            <span className="text-[#F29100] transition-colors group-hover:text-[#8A5100]">
              <ArrowIcon />
            </span>
          </div>
        </div>
